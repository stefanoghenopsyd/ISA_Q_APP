...
def main():
    ...
    if not st.session_state.submitted:
        with st.expander("ℹ️ Informazioni", expanded=False):
            st.write("ISA-Q è lo "Strumento Zero" di GÉNERA. È il punto di ingresso per un percorso di sviluppo personale delle soft skill d’impatto. Il suo obiettivo è di fornire una valutazione preliminare di ciò che chiamiamo ”indice di impatto personale” (Persona Impact Index), attraverso la misurazione della percezione soggettiva delle 5 dimensioni concettuali del modello GÉNERA: Autonomia e competenza (A&C); Potere personale (S-E); Coerenza e significato (C&M); Impatto e futuro (I&F); Flessibilità evolutiva (F-A). Il test si svolge compilando un questionario composto da 15 item totali, espressi sotto forma di affermazioni riferibili a comuni esperienze di vita nel contesto lavorativo. Il compilatore dovrà valutare il proprio livello di accordo con ciascuna affermazione su una scala a 6 punti (da 1 = non sono per nulla d’accordo con l’affermazione; 6 = sono totalmente d’accordo con l’affermazione). Il punteggio del test si ottiene sommando il punteggio ottenuto in ognuna delle 5 dimensioni. Il test è totalmente anonimo, per una più efficace interpretazione dei dati le 15 affermazioni sono precedute da 3 item anagrafici relativi al genere, età e scolarità. Il compilatore riceverà al termine un feedback personale immediato sia di tipo narrativo, sia sotto forma di immagine. I dati dei rispondenti saranno conservati in modo sicuro ai sensi delle norme vigenti e saranno trattati esclusivamente in forma aggregata per finalità di ricerca scientifica. Compila il questionario per scoprire il tuo profilo d'impatto.")
