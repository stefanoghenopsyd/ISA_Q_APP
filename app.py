...
def main():
    ...
    if not st.session_state.submitted:
        with st.expander("ℹ️ Informazioni", expanded=False):
            st.write("ISA-Q è lo 'Strumento Zero' di GÉNERA: il punto di ingresso per un percorso di sviluppo personale delle soft skill d’impatto. 
            Il suo obiettivo è di fornire una valutazione preliminare di ciò che chiamiamo 'Indice di Impatto Personale' (Persona Impact Index), attraverso la misurazione della percezione soggettiva delle 5 dimensioni del modello GÉNERA: Autonomia e competenza (A&C); Potere personale (S-E); Coerenza e significato (C&M); Impatto e futuro (I&F); Flessibilità evolutiva (F-A). 
