...
DOMANDE = [\
    ...
    \{\
        "id": "q1", \
        "testo": "Sento che le mie attivit\'e0 quotidiane riflettono i miei valori e le mie convinzioni personali.", \
        "dim": 0, \
        "reverse": False\
    \},\
    \{\
        "id": "q2", \
        "testo": "Quando affronto un compito difficile, ho fiducia nella mia capacit\'e0 di padroneggiarlo con le risorse a mia disposizione.", \
        "dim": 0, \
        "reverse": False\
    \},\
    \{\
        "id": "q3", \
        "testo": "Mi capita spesso di svolgere il mio lavoro solo perch\'e9 mi \'e8 stato imposto, senza sentirlo veramente 'mio'.", \
        "dim": 0, \
        "reverse": True # (R)\
    \},\
\
    ...
    \{\
        "id": "q5", \
        "testo": "Sento di avere l'autorit\'e0 e la capacit\'e0 necessarie per influenzare i processi decisionali che riguardano il mio ruolo.", \
        "dim": 1, \
        "reverse": False\
    \},\
    \{\
        "id": "q6", \
        "testo": "Ho la sensazione che la mia crescita professionale dipenda pi\'f9 dal caso o dalle decisioni altrui che dal mio impegno.", \
        "dim": 1, \
        "reverse": True # (R)\
    \}, \
\
...
\
    ...
    \{\
        "id": "q10", \
        "testo": "Mi impegno attivamente per trasmettere le mie conoscenze e competenze ai colleghi o ai collaboratori pi\'f9 giovani.", \
        "dim": 3, \
        "reverse": False\
    \},\
    \{\
        "id": "q11", \
        "testo": "Mi d\'e0 soddisfazione sapere che i risultati del mio lavoro avranno un impatto positivo sugli altri anche in futuro.", \
        "dim": 3, \
        "reverse": False\
    \},\
    \{\
    ...
    \},\
\
    # --- DIMENSIONE 5: Flessibilit\'e0 evolutiva (F-A) ---\
    \{\
        "id": "q13", \
    ...
    \{\
        "id": "q14", \
        "testo": "Considero i cambiamenti organizzativi inattesi come opportunit\'e0 per sviluppare nuove competenze.", \
        "dim": 4, \
        "reverse": False\
    \},\
    \{\
...
OPT_ETA = ["Meno di 20 anni", "21-30 anni", "31-40 anni", "41-50 anni", "51-60 anni", "61-70 anni", "Pi\'f9 di 70 anni"]\
OPT_SCOLARITA = ["Licenza media", "Qualifica professionale", "Diploma di maturit\'e0", "Laurea triennale", "Laurea magistrale, specialistica o a ciclo unico", "Diploma post laurea"]\
\
...
def get_feedback_narrativo(totale):\
    ...
    if totale < 31:\
        ...
        if totale < 16:\
            ...
            dett = "Il potenziale \'e8 ancora del tutto inespresso o bloccato."\
        else:\
            desc = "**Parzialmente Latente**"\
            dett = "Si intravedono i primi segnali, ma l'impatto non \'e8 ancora tangibile."\
            \
    elif 31 <= totale <= 54:\
        colore = "#FFC300" # Giallo\
        livello = "IMPATTO EMERGENTE"\
        # Range 31-54 (24 pt). Met\'e0 \'e8 42/43.\
        if totale <= 42:\
            ...
            dett = "L'impatto \'e8 visibile e chiaro, anche se forse non ancora sistemico."\
            \
    else: # >= 55\
        colore = "#4CAF50" # Verde\
        livello = "IMPATTO GENERATIVO"\
        # Range 55-75 (20 pt). Met\'e0 \'e8 65.\
        if totale < 65:\
            desc = "**Inizialmente Generativo**"\
            dett = "Il soggetto produce valore autonomamente e con costanza."\
        else:\
            desc = "**Compiutamente Generativo**"\
            dett = "L'impatto \'e8 sistemico, sostenibile e moltiplicatore."\
            \
...
def plot_radar_chart(punteggi_dim, colore_riempimento):\
    ...
    ax.set_yticklabels([])\
    \
    # Scala fissa: max possibile \'e8 15 (3 domande * 5 pt). Impostiamo 16 per margine.\
    ax.set_ylim(0, 16)\
    \
...
\
def main():\
    st.set_page_config(page_title="Genera - Questionario", page_icon="\uc0\u11093 ", layout="centered")\
    \
    # --- HEADER E LOGO ---\
...
\
    ...
    with st.expander("\uc0\u8505 \u65039  Informazioni sul Questionario (Obiettivi e Metodologia)", expanded=False):\
        st.markdown("""\
        ### Obiettivi\
        Il presente strumento \'e8 stato ideato per mappare e restituire un'immagine chiara della tua capacit\'e0 di **generare impatto** nel contesto attuale. L'obiettivo \'e8 fornirti consapevolezza sulle tue leve di forza e sulle aree di potenziale sviluppo.\
\
        ### Struttura\
        Il modello si fonda su **5 Dimensioni** chiave che rappresentano i pilastri dell'impatto personale (Autonomia, Potere, Coerenza, Impatto Futuro, Flessibilit\'e0). Il feedback finale sar\'e0 restituito attraverso un **profilo grafico** (un poligono iscritto in un cerchio) e un **profilo narrativo**.\
\
        ### Metodologia\
        Il questionario si compone di **15 item** pi\'f9 una breve sezione anagrafica.\
        Per ogni affermazione, ti chiediamo di esprimere il tuo grado di accordo su una scala da 1 ("Per nulla") a 5 ("Moltissimo").\
        *Non esistono risposte giuste o sbagliate: rispondi con sincerit\'e0 per ottenere un profilo il pi\'f9 possibile autentico.*\
        """)\
\
    st.markdown("---")\
\
    ...
    with st.form("questionario_form"):\
        ...
        with col1:\
            ...
            eta = st.selectbox("Et\'e0", OPT_ETA)\
        with col2:\
            scolarita = st.selectbox("Scolarit\'e0", OPT_SCOLARITA)\
            \
...
\
            with col_dx:\
                ...
                st.info(f"\{dettaglio\}", icon="\uc0\u55357 \u56481 ")\
                \
                st.markdown("---")\
